...
def write_xml(generation: ResolvedGeneration, capability: dict[str, object], xsd_path: Path, source_sha256: str) -> tuple[bytes, dict[str, object]]:
    ...
    if generation.classes:
        ...
        for index, item in enumerate(generation.classes, 1):
            ...
            _rdf_value(element, "identifierInNamespace", item.identifier, item.resource, item.source_triple_ids, item.mapping_rule, "rdf", f"{base}/identifierInNamespace", entries)
            ...
            _relations(element, item.relations, ("subClassOf", "equivalentClass"), base, entries)
    ...
    xsd = capability["xsd"]
    trace = {"format_version": "1.1", "source_sha256": source_sha256, "xsd": {"version": xsd["version"], "sha256": xsd["sha256"]}, "xml_sha256": xml_sha256, "entries": entries}
    return xml, trace
...
def _reference_element(parent: etree._Element, name: str, reference: ResolvedReference, path: str, entries: list[dict[str, object]]) -> None:
    ...
    _rdf_value(parent, name, reference.value, reference.resource, reference.triple_ids, reference.mapping_rule, reference.origin, path, entries, attributes)


def _rdf_value(parent: etree._Element, name: str, value: str, resource: object, triple_ids: tuple[str, ...], mapping_rule: str, origin: str, path: str, entries: list[dict[str, object]], attributes: dict[str, str] | None = None) -> None:
    attributes = attributes or {}
    element = etree.SubElement(parent, name, **attributes)
    element.text = value
    _entry(entries, path, name, value, attributes, resource, triple_ids, mapping_rule, origin)


def _entry(entries: list[dict[str, object]], path: str, element: str, value: str, attributes: dict[str, str], resource: object, triple_ids: tuple[str, ...], mapping_rule: str, origin: str) -> None:
