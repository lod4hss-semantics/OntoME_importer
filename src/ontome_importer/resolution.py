...
def resolve_generation(
    ...
    for resource in sorted((item.id for item in inventory.resources if item.id.kind == "uri"), key=lambda term: term.value):
        ...
        if not rules:
            continue
        if len(rules) != 1:
            findings.append(_issue("invalid", resource, (), None, "More than one generation mapping rule applies."))
            continue
        target = rules[0]["target"]
        bases.append((resource, rules[0], target))

    identifiers: dict[RdfTerm, str] = {}
    for resource, rule, target in bases:
        prefix = target["identifier_in_namespace"]["strip_prefix"]
        if not resource.value.startswith(prefix):
            findings.append(_issue("blocked", resource, (), rule["id"], "Identifier URI does not start with the configured strip_prefix."))
            continue
        identifier = resource.value.removeprefix(prefix)
        if not identifier or any(character.isspace() for character in identifier) or "{" in identifier or "}" in identifier:
            findings.append(_issue("blocked", resource, (), rule["id"], "Local identifier is empty or invalid."))
            continue
        identifiers[resource] = identifier
    collisions = Counter(identifiers.values())
    for resource, identifier in identifiers.items():
        if collisions[identifier] > 1:
            findings.append(_issue("invalid", resource, (), None, f"Local identifier collision: {identifier}"))

    resolved_classes: list[ResolvedClass] = []
    resolved_properties: list[ResolvedProperty] = []
    used_namespaces: set[int] = set()
    for resource, rule, target in bases:
        if resource not in identifiers or collisions[identifiers.get(resource, "")] > 1:
            continue
        ...
        if not labels:
            findings.append(_issue("blocked", resource, (), rule["id"], "At least one localized label is required."))
        ...
        if target["entity_kind"] == "property":
            ...
            domain = _single_reference(source_triples, domain_predicate, resource, rule["id"], identifiers, mapping, registry, findings, used_namespaces, "domain")
            range_ = _single_reference(source_triples, range_predicate, resource, rule["id"], identifiers, mapping, registry, findings, used_namespaces, "range")
        _check_unhandled(source_triples, handled, resource, rule["id"], findings)
        if target["entity_kind"] == "class":
            resolved_classes.append(ResolvedClass(resource, _type_triple_ids(source_triples), rule["id"], identifiers[resource], _identifier_uri(target, resource), labels, relations, texts))
        else:
            resolved_properties.append(ResolvedProperty(resource, _type_triple_ids(source_triples), rule["id"], target["property_kind"], identifiers[resource], _identifier_uri(target, resource), labels, relations, domain, range_, texts))
...
def _texts(triples: list[InventoryTriple], predicates: list[str], resource: RdfTerm, rule: str, findings: list[dict[str, object]], name: str) -> tuple[ResolvedText, ...]:
    ...
    for triple in triples:
        ...
        if triple.object.kind != "literal" or not triple.object.language:
            findings.append(_issue("blocked", resource, (triple.id,), rule, f"{name} must be a literal with a language."))
            continue
...
def _configured_texts(triples: list[InventoryTriple], target: dict[str, object], resource: RdfTerm, rule: str, findings: list[dict[str, object]]) -> tuple[tuple[tuple[str, ResolvedText], ...], set[str]]:
    ...
    return tuple(sorted(values, key=lambda item: (item[0], item[1].language, item[1].value))), predicates
...
def _single_reference(triples: list[InventoryTriple], predicate: str, resource: RdfTerm, rule: str, identifiers: dict[RdfTerm, str], mapping: dict[str, object], registry: dict[str, object], findings: list[dict[str, object]], used_namespaces: set[int], label: str) -> ResolvedReference | None:
    matches = [triple for triple in triples if triple.predicate.value == predicate]
    if len(matches) != 1:
        findings.append(_issue("blocked", resource, tuple(item.id for item in matches), rule, f"Property requires exactly one named {label}."))
        return None
...
def _reference(triple: InventoryTriple, resource: RdfTerm, rule: str, identifiers: dict[RdfTerm, str], mapping: dict[str, object], registry: dict[str, object], findings: list[dict[str, object]], used_namespaces: set[int]) -> ResolvedReference | None:
    if triple.object.kind != "uri":
        findings.append(_issue("blocked", resource, (triple.id,), rule, "Reference target must be a named URI."))
        return None
    if triple.object in identifiers:
        return ResolvedReference(identifiers[triple.object], None, triple.object, (triple.id,), rule, "rdf")
    external = next((item for item in mapping["external_references"] if item["uri"] == triple.object.value), None)
    if external is None:
        findings.append(_issue("blocked", resource, (triple.id,), rule, "External reference is absent from external_references."))
        return None
    ...
    if namespace["status"] == "forbidden":
        findings.append(_issue("blocked", resource, (triple.id,), rule, "External reference namespace is forbidden."))
        return None
    used_namespaces.add(external["reference_namespace"])
    if namespace["status"] == "deprecated":
        findings.append(_issue("resolved", resource, (triple.id,), rule, "Deprecated external namespace used."))
    return ResolvedReference(external["identifier"], external["reference_namespace"], triple.object, (triple.id,), rule, "external_reference")
...
def _check_entity_type(resource: RdfTerm, target: dict[str, object], triples: list[InventoryTriple], rule: str, findings: list[dict[str, object]]) -> None:
    ...
    if not types & expected[kind]:
        findings.append(_issue("blocked", resource, (), rule, f"Resource does not have the required {kind} RDF type."))
...
def _issue(status: str, resource: RdfTerm, triple_ids: tuple[str, ...], rule: str | None, reason: str) -> dict[str, object]:
    result: dict[str, object] = {"status": status, "resource": resource.to_dict(), "triple_ids": list(sorted(triple_ids)), "reason": reason}
    if rule:
...
def _audit_document(inventory: Inventory, capability: dict[str, object], findings: list[dict[str, object]]) -> dict[str, object]:
    ...
    xsd = capability["xsd"]
    return {"format_version": "1.0", "source": {"file": inventory.source_file, "sha256": inventory.source_sha256}, "xsd": {"version": xsd["version"], "sha256": xsd["sha256"]}, "strict_ok": not any(item["status"] in {"blocked", "invalid", "configured"} for item in findings), "findings": findings, "counts": dict(sorted(counts.items()))}
