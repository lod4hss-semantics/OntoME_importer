...
The audit is generic. It detects the RDF, RDFS, OWL and SKOS subset defined in the [Semantic Policy](semantic-policy.md), preserves unknown predicates and RDF types, and links each finding to stable inventory triples. Capability and mapping profiles determine whether a finding is mapped, configured, excluded, blocked, or invalid.
...
A generic `map` target is reported as `configured`, not `mapped`: phase 3 cannot claim XML representability until a typed generation mapping validates it. A generation mapping must also cover each exportable assertion predicate; a resource rule alone is insufficient. Namespaces registered as `forbidden` always block. Every external relation target, including XSD terms, requires an exact URI-to-OntoME reference configuration before generation.
